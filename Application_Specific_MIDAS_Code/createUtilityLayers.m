...
for indexI = 1:length(locations)
    ...
    for indexJ = 1:length(utilityLayerFunctions)
        for indexK = 1:quantiles
            ...
            a = max(0,(1 + incomeShockPercentPerFoot_0(ceil(indexJ/quantiles)) / 100 * floodShock));
            a = a .* max(0,(1 + incomeShockPercentPerFoot(ceil(indexJ/quantiles)) / 100 * floodShock_0));
