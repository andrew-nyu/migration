...
discountFactor = 1./((1+agent.discountRate).^(0:agent.numPeriodsEvaluate-1)');
...
for indexL = 1:length(locationList)
    ...
    for indexP = currentPortfolio:totalNumPortfolios
        nextRandom = createPortfolio(find(any(agent.knowsIncomeLocation(locationList(indexL),:),1)),utilityVariables.utilityTimeConstraints);
        ...
        while(~isempty(blankElements))
            ...
            if(isnan(portfolioData(currentLayer,blankElements(1))))
                ...
                if(~isempty(availableSamples))
                    ...
                    sampleIndex = sampleIndex(randperm(length(sampleIndex),1));
                    
                    %identify any additional layers that can be filled in
                    %with this sample
                    currentSample = fullHistory(priorityList(indexB:end),sampleIndex);
                    currentPeriod = portfolioData(priorityList(indexB:end),blankElements(1));
                    filledHoles = logical(isnan(currentPeriod) .* ~isnan(currentSample));
                    
                    %fill the holes and then replace the current sample in
                    %the evaluation period with this more complete set
                    currentPeriod(filledHoles) = currentSample(filledHoles);
                    portfolioData(priorityList(indexB:end),blankElements(1)) = currentPeriod;
                end
            end
